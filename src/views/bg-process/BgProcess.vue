...
import { BGPROCESS } from '@/store/bg-process/constants';
import DataTable from '@/components/itl-dataTable-files/itl-dataTable/DataTable.vue';
import { bgProcessVariables, filterModal } from './bgProcess';
...
const dataVariables = bgProcessVariables;
const getVendorDataStatus = computed(() => store.getters[`${BGPROCESS.NAME}/getVendorStatus`]);
dataVariables.value.exportStatus = computed(() => store.getters[`${BGPROCESS.NAME}/getExportStatus`]);
const savedFilteredList = computed(() => store.getters[`${BGPROCESS.NAME}/getViewSavedFilteredData`]);
dataVariables.value.dataTableData = computed(() => store.getters[`${BGPROCESS.NAME}/getBillingData`]);
// dataVariables.value.getWidgetTitle = computed(() => store.getters[`${BGPROCESS.NAME}/getWidgetTitle`]);
const getPaginatorLastValue = computed(() => store.getters[`${BGPROCESS.NAME}/getPaginatorLastValue`]);
const getVendorModalPayload = computed(() => store.getters[`${BGPROCESS.NAME}/getVendorModalPayload`]);
dataVariables.value.allFilterData = computed(() => store.getters[`${BGPROCESS.NAME}/getAllFilterData`]);
dataVariables.value.getVendorValues = computed(() => store.getters[`${BGPROCESS.NAME}/getVendorValues`]);
dataVariables.value.getWidgetValues = computed(() => store.getters[`${BGPROCESS.NAME}/getWidgetValues`]);
const getPaginatorStartValue = computed(() => store.getters[`${BGPROCESS.NAME}/getPaginatorStartValue`]);
dataVariables.value.getUpdatedColumns = computed(() => store.getters[`${BGPROCESS.NAME}/getUpdatedColumns`]);
dataVariables.value.saveFilterResponse = computed(() => store.getters[`${BGPROCESS.NAME}/getSaveFilterResponse`]);
dataVariables.value.getMappedFilterValue = computed(() => store.getters[`${BGPROCESS.NAME}/getMappedFilterValue`]);
dataVariables.value.getUpdatedSizePayload = computed(() => store.getters[`${BGPROCESS.NAME}/getUpdatedSizePayload`]);
dataVariables.value.savedDatatableResponse = computed(() => store.getters[`${BGPROCESS.NAME}/getSavedDatatableResponse`]);

onMounted(() => {
    dataVariables.value.selectedTabHeading = 'Order Processes';
...
onMounted(async () => {
    ...
    document.body.classList.add('order-processes-dt');
    store.commit(`${BGPROCESS.NAME}/clearAllFilter`);
...
onBeforeMount(async () => {
    ...
    dataTableFncs.getCurrentRoutes();
    await dataTableFncs.getCurrentModuleStoreName(BGPROCESS.NAME);
});
const formatDate = async () => {
    await store.dispatch(`${BGPROCESS.NAME}/getDateRange`, { [dataVariables.value.syncDateName]: dataTableFncs.changeDateFormatPayload() });
    // await store.dispatch(`${BGPROCESS.NAME}/getDateRange`, dataTableFncs.changeDateFormatPayload());
};

const vendorModalVisible = (value) => (dataVariables.value.isVendorModalVisible = value);
const setExportValue = (val) => store.dispatch(`${BGPROCESS.NAME}/getSelectedRow`, val);
provide('storeName', BGPROCESS);
onMounted(() => {
