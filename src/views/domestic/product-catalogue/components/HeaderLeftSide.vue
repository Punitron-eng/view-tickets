...
watch(
    () => productCatalogueVariable.value.getVendorValues,
    async (newVal, oldVal) => {
        // await applyChannel();
        console.log(newVal, '✅✅', newVal.id.length);
...
const fetchChannelsAndStores = async () => {
    ...
    const res = await store.dispatch(`${PRODUCTCATALOGUE.NAME}/fetchChannels`, { module: 1, vendor_id: temp });
    if (res.status == 'success') {
        productCatalogueVariable.value.channelOptions = res.data;
        productCatalogueVariable.value.channel = res.data[0];

        // productCatalogueVariable.value.channelOptions = [{id: 1, value: 'ciuhtfu'}];
        // productCatalogueVariable.value.channel = productCatalogueVariable.value.channelOptions[0];
        ...
        if (checkUserType('vendor') ? true : productCatalogueVariable.value.getVendorValues.id.length == 1) {
            ...
            if (fetchedStores.status == 'success') {
                ...
                // productCatalogueVariable.value.storeOptions = [{id: 1, value: 'ciuhtfu'}];
                // productCatalogueVariable.value.store = productCatalogueVariable.value.storeOptions[0];
