...
import { NDRCALLDETAILS } from '@/store/domestic/admin-pages/ndr-call-details/constants';
import DataTable from '@/components/itl-dataTable-files/itl-dataTable/DataTable.vue';
import { ndrCallDetailVariables, filterModal } from './ndrCallDetail';
...
const dataVariables = ndrCallDetailVariables;
const getVendorDataStatus = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getVendorStatus`]);
dataVariables.value.exportStatus = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getExportStatus`]);
const savedFilteredList = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getViewSavedFilteredData`]);
dataVariables.value.dataTableData = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getBillingData`]);
// dataVariables.value.getWidgetTitle = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getWidgetTitle`]);
const getPaginatorLastValue = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getPaginatorLastValue`]);
const getVendorModalPayload = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getVendorModalPayload`]);
dataVariables.value.allFilterData = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getAllFilterData`]);
dataVariables.value.getVendorValues = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getVendorValues`]);
dataVariables.value.getWidgetValues = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getWidgetValues`]);
const getPaginatorStartValue = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getPaginatorStartValue`]);
dataVariables.value.getUpdatedColumns = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getUpdatedColumns`]);
dataVariables.value.saveFilterResponse = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getSaveFilterResponse`]);
dataVariables.value.getMappedFilterValue = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getMappedFilterValue`]);
dataVariables.value.getUpdatedSizePayload = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getUpdatedSizePayload`]);
dataVariables.value.savedDatatableResponse = computed(() => store.getters[`${NDRCALLDETAILS.NAME}/getSavedDatatableResponse`]);

onMounted(() => {
    dataVariables.value.selectedTabHeading = 'NDR Call Details';
...
onMounted(async () => {
    ...
    document.body.classList.add('ndr-call-detail');
    store.commit(`${NDRCALLDETAILS.NAME}/clearAllFilter`);
...
onBeforeMount(async () => {
    dataVariables.value.moduleName = 'ndr_call_details';
    dataVariables.value.syncDateName = 'order_date';
    dataTableFncs.getCurrentRoutes();
    await dataTableFncs.getCurrentModuleStoreName(NDRCALLDETAILS.NAME);
});
const formatDate = async () => {
    await store.dispatch(`${NDRCALLDETAILS.NAME}/getDateRange`, { [dataVariables.value.syncDateName]: dataTableFncs.changeDateFormatPayload() });
    // await store.dispatch(`${NDRCALLDETAILS.NAME}/getDateRange`, dataTableFncs.changeDateFormatPayload());
};

const vendorModalVisible = (value) => (dataVariables.value.isVendorModalVisible = value);
const setExportValue = (val) => store.dispatch(`${NDRCALLDETAILS.NAME}/getSelectedRow`, val);
provide('storeName', NDRCALLDETAILS);
onMounted(() => {
