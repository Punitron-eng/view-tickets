...
import { CANCELRTOPODREQINTL } from '@/store/domestic/admin-pages/cancel-rto-pod-request-intl/constants';
import { cancelRtoPodIntlVariables, filterModal } from './cancelRtoPodRequestIntl.js';
...
const dataVariables = cancelRtoPodIntlVariables;
const getPaginatorStartValue = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getPaginatorStartValue`]);
const getPaginatorLastValue = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getPaginatorLastValue`]);
const savedFilteredList = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getViewSavedFilteredData`]);
const getVendorModalPayload = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getVendorModalPayload`]);
const getVendorDataStatus = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getVendorStatus`]);
dataVariables.value.allFilterData = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getAllFilterData`]);
dataVariables.value.dataTableData = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getDtData`]);
dataVariables.value.getMappedFilterValue = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getMappedFilterValue`]);
dataVariables.value.getUpdatedColumns = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getUpdatedColumns`]);
dataVariables.value.getUpdatedSizePayload = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getUpdatedSizePayload`]);
dataVariables.value.savedDatatableResponse = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getSavedDatatableResponse`]);
dataVariables.value.saveFilterResponse = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getSaveFilterResponse`]);
dataVariables.value.exportStatus = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getExportStatus`]);
dataVariables.value.getVendorValues = computed(() => store.getters[`${CANCELRTOPODREQINTL.NAME}/getVendorValues`]);

onMounted(() => {
    dataVariables.value.selectedTabHeading = 'Cancel RTO POD Request INTL';
...
onMounted(async () => {
    ...
    store.commit(`${CANCELRTOPODREQINTL.NAME}/clearAllFilter`);
...
onBeforeMount(async () => {
    dataVariables.value.moduleName = 'cancel_refund_intl';
    dataVariables.value.syncDateName = 'order_date';
    dataTableFncs.getCurrentRoutes();
    await dataTableFncs.getCurrentModuleStoreName(CANCELRTOPODREQINTL.NAME);
});
const formatDate = async () => {
    await store.dispatch(`${CANCELRTOPODREQINTL.NAME}/getDateRange`, { [dataVariables.value.syncDateName]: dataTableFncs.changeDateFormatPayload() });
   
    // await store.dispatch(`${CANCELRTOPODREQINTL.NAME}/getDateRange`, dataTableFncs.changeDateFormatPayload());
};

const vendorModalVisible = (value) => (dataVariables.value.isVendorModalVisible = value);
const setExportValue = (val) => store.dispatch(`${CANCELRTOPODREQINTL.NAME}/getSelectedRow`, val);
provide('storeName', CANCELRTOPODREQINTL);
onMounted(() => {
