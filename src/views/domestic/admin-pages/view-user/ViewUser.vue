...
import { ADMINVIEWUSER } from '@/store/domestic/admin-pages/view-user/constants';
import { viewUserAdminVariables, filterModal } from './viewUser.js';
...
const dataVariables = viewUserAdminVariables;
const getPaginatorStartValue = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getPaginatorStartValue`]);
const getPaginatorLastValue = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getPaginatorLastValue`]);
const savedFilteredList = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getViewSavedFilteredData`]);
const getVendorModalPayload = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getVendorModalPayload`]);
const getVendorDataStatus = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getVendorStatus`]);
dataVariables.value.allFilterData = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getAllFilterData`]);
dataVariables.value.dataTableData = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getDtData`]);
dataVariables.value.getMappedFilterValue = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getMappedFilterValue`]);
dataVariables.value.getUpdatedColumns = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getUpdatedColumns`]);

dataVariables.value.getUpdatedSizePayload = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getUpdatedSizePayload`]);
dataVariables.value.savedDatatableResponse = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getSavedDatatableResponse`]);
dataVariables.value.saveFilterResponse = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getSaveFilterResponse`]);
dataVariables.value.exportStatus = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getExportStatus`]);
dataVariables.value.getVendorValues = computed(() => store.getters[`${ADMINVIEWUSER.NAME}/getVendorValues`]);
...
onMounted(async () => {
    document.body.classList.add('view-user-admin-wrapper');
    store.commit(`${ADMINVIEWUSER.NAME}/clearAllFilter`);
...
onBeforeMount(async () => {
    dataVariables.value.moduleName = 'user';
    dataVariables.value.syncDateName = 'user_created_date';
    dataTableFncs.getCurrentRoutes();
    await dataTableFncs.getCurrentModuleStoreName(ADMINVIEWUSER.NAME);
});
const formatDate = async () => {
    await store.dispatch(`${ADMINVIEWUSER.NAME}/getDateRange`, dataTableFncs.changeDateFormatPayload());
};

const vendorModalVisible = (value) => (dataVariables.value.isVendorModalVisible = value);
const setExportValue = (val) => store.dispatch(`${ADMINVIEWUSER.NAME}/getSelectedRow`, val);
provide('storeName', ADMINVIEWUSER);
onMounted(() => {
...
.view-user-admin-wrapper .p-datatable-wrapper {
    min-height: 50vh;
    max-height: 65vh;
}
</style>
