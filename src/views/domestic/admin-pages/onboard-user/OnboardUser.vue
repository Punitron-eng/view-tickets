...
import { ONBOARDUSER } from '@/store/domestic/admin-pages/onboard-user/constants';
import { onboardUserVariables, filterModal } from './onboardUser.js';
...
const dataVariables = onboardUserVariables;
const getPaginatorStartValue = computed(() => store.getters[`${ONBOARDUSER.NAME}/getPaginatorStartValue`]);
const getPaginatorLastValue = computed(() => store.getters[`${ONBOARDUSER.NAME}/getPaginatorLastValue`]);
const savedFilteredList = computed(() => store.getters[`${ONBOARDUSER.NAME}/getViewSavedFilteredData`]);
const getVendorModalPayload = computed(() => store.getters[`${ONBOARDUSER.NAME}/getVendorModalPayload`]);
const getVendorDataStatus = computed(() => store.getters[`${ONBOARDUSER.NAME}/getVendorStatus`]);
dataVariables.value.allFilterData = computed(() => store.getters[`${ONBOARDUSER.NAME}/getAllFilterData`]);
dataVariables.value.dataTableData = computed(() => store.getters[`${ONBOARDUSER.NAME}/getDtData`]);
dataVariables.value.getMappedFilterValue = computed(() => store.getters[`${ONBOARDUSER.NAME}/getMappedFilterValue`]);
dataVariables.value.getUpdatedColumns = computed(() => store.getters[`${ONBOARDUSER.NAME}/getUpdatedColumns`]);
dataVariables.value.getUpdatedSizePayload = computed(() => store.getters[`${ONBOARDUSER.NAME}/getUpdatedSizePayload`]);
dataVariables.value.savedDatatableResponse = computed(() => store.getters[`${ONBOARDUSER.NAME}/getSavedDatatableResponse`]);
dataVariables.value.saveFilterResponse = computed(() => store.getters[`${ONBOARDUSER.NAME}/getSaveFilterResponse`]);
dataVariables.value.exportStatus = computed(() => store.getters[`${ONBOARDUSER.NAME}/getExportStatus`]);
dataVariables.value.getVendorValues = computed(() => store.getters[`${ONBOARDUSER.NAME}/getVendorValues`]);
// dataVariables.value.getWidgetTitle = computed(() => store.getters[`${ONBOARDUSER.NAME}/getWidgetTitle`]);
// dataVariables.value.getWidgetValues = computed(() => store.getters[`${ONBOARDUSER.NAME}/getWidgetValues`]);
const storeId = computed(() => store.getters[`${ONBOARDUSER.NAME}/sendStoreId`]);

onMounted(() => {
    dataVariables.value.selectedTabHeading = 'Onboard User';
...
provide('storeName', ONBOARDUSER);
onMounted(async () => {
    // await dataTableFncs.getAllWidgetTitleData();
    // document.body.classList.add('dt-wrapper-outer');
    document.body.classList.add('onboard-user');
    store.commit(`${ONBOARDUSER.NAME}/clearAllFilter`);
...
onBeforeMount(async () => {
    dataVariables.value.moduleName = 'onboard_user';
    dataVariables.value.syncDateName = 'calling_date';
    dataTableFncs.getCurrentRoutes();
    await dataTableFncs.getCurrentModuleStoreName(ONBOARDUSER.NAME);
});
const formatDate = async () => {
    await store.dispatch(`${ONBOARDUSER.NAME}/getDateRange`, { [dataVariables.value.syncDateName]: dataTableFncs.changeDateFormatPayload() });

};

const vendorModalVisible = (value) => (dataVariables.value.isVendorModalVisible = value);
const setExportValue = (val) => store.dispatch(`${ONBOARDUSER.NAME}/getSelectedRow`, val);
onMounted(() => {
