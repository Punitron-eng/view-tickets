...
const emits = defineEmits(['showOtpModalOrNot']);
...
const getMobile = computed(() => store.getters[`${RATECHART.NAME}/getMobile`]);
...
onMounted(async () => {
    if (topHeader.value.user_type == 3) {
        ...
        if (localStorage.getItem('showOtpModal') == 'false') {
            visible.value = false;
        } else {
            visible.value = true;
        }
...
const verifyOtp = async () => {
    if (isAllOtpFields.value) {
        ...
        const res = await store.dispatch(`${RATECHART.NAME}/verifyOtp`, { submit_source: 'otp_modal', step_no: 1, otp_1: otp1.value, otp_2: otp2.value, otp_3: otp3.value, otp_4: otp4.value, mobile_token: cookies.get('mobile_token') });
        ...
        if (res.status == 'error') {
            ...
            store.dispatch(`${RATECHART.NAME}/setIsSendOtp`, 1);
            localStorage.setItem('showOtpModal', false);
            emits('showOtpModalOrNot', 'false');
            const res1 = await store.dispatch(`${RATECHART.NAME}/fetchDefaultData`, { mobile_token: cookies.get('mobile_token') });
        }
...
const resendOtp = async () => {
    ...
    minutes.value = 2;
    seconds.value = 0;
    // const res = await store.dispatch(`${RATECHART.NAME}/sendOtp`, { mobile_token: cookies.get('mobile_token') });
...
watch(
    () => isSendOtp.value,
    () => {
        visible.value = (localStorage.getItem('showOtpModal') != 'false') && topHeader.value.user_type == 3 ? true : false;
    }
);
...
<template>
    <div class="verify-otp" id="verify-otp">
        <DialogView
            ...
            modal
            :draggable="false"
            :style="{ width: '40rem', margin: 'auto 0 !important', height: 'auto !important' }"
