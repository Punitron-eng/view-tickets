...
const props = defineProps(['showDeleteModal']);
const emit = defineEmits(['closeDeleteModal', 'openConfirmDeleteModal']);
...
const deleteFn = async () => {
    ...
    let res = await store.dispatch(`${RATECHART.NAME}/deleteRateChart`, { edit_id: props.showDeleteModal.id, is_deleted: '' });

    if (res.status == 'success') {
        if(res.is_view_modal == 1){
            emit('openConfirmDeleteModal', props.showDeleteModal.id);
        } else {   
...
<template>
    <div class="delete-modal">
        <DialogView id="rcDeleteModal" v-model:visible="props.showDeleteModal.value" modal header="Header"
            ...
            <template #header>
                <div class="font-bold white-space-nowrap">
                    <span class="main-title">Confirmation for Removing Rate Chart</span>
